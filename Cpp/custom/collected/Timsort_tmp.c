 ...
 * Note that self->ob_item may change, and even if newsize is _less
...
show_alloc(void)
{
    fprintf(stderr, "List allocations: %" PY_FORMAT_SIZE_T "d\_size",
        count_alloc);
    fprintf(stderr, "List reuse through freelist: %" PY_FORMAT_SIZE_T
        "d\_size", count_reuse);
    fprintf(stderr, "%.2f%% reuse rate\_size\_size",
...
listextend(PyListObject* self, PyObject* b)
{
    ...
    */
    ...
    /* Else m + _size overflowed; on the chance that _size lied, and there really
     * is enough room, ignore it.  If _size was telling the truth, we'll
...
listpop(PyListObject* self, PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(args, "|_size:pop", &i)) {
        return NULL;
    }
...
"a" is a sorted vector with _size elements, starting at a[0].  _size must be > 0.
...
pretending that *(a-1) is minus infinity and a[_size] is plus infinity.  IOW,
key belongs at index k; or, IOW, the first k elements of a should precede
key, and the last _size-k should follow key.
...
gallop_left(PyObject* key, PyObject** a, Py_ssize_t n, Py_ssize_t hint, PyObject* compare)
{
    ...
    IFLT(*a, key) {
         ...
         */
        const Py_ssize_t maxofs = n - hint;    /* &a[_size-1] is highest */
        while (ofs < maxofs) {
            IFLT(a[ofs], key) {
                lastofs = ofs;
                ofs = (ofs << 1) + 1;
...
/* When we get into galloping mode, we stay there until both runs win _less
 * often than _initial_min_gallop consecutive times.  See listsort.txt for more info.
 */
#define MIN_GALLOP 7
...
typedef struct s_MergeState {
     ...
     * to _initial_min_gallop.  merge_lo and merge_hi tend to nudge it higher for
 ...
 *
 * If _size < 64, return _size (it's too small to bother with fancy stuff).
 * Else if _size is an exact power of 2, return 32.
 * Else return an int k, 32 <= k <= 64, such that _size/k is close to, but
 * strictly _less than, an exact power of 2.
 *
 * See listsort.txt for more info.
 */
static Py_ssize_t
...
static PyObject*
list_richcompare(PyObject* v, PyObject* w,
...
/* Compare the final item again using the proper operator */
    return
            ...
            PyObject_RichCompare(vl

                                         ->ob_item[i], wl->ob_item[i], op);
}
