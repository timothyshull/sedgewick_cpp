...
int main(int argc, char* argv[])
{
    ...
    Adj_matrix_edge_weighted_digraph G{num_vertices};
    for (int i = 0; i < num_edges; ++i) {
        ...
        if (v == w) { G.add_edge(v, w, std::abs(weight)); }
        else { G.add_edge(v, w, weight); }
    }

    Std_out::print_line(G);

    Floyd_warshall spt{G};
    ...
    for (int v = 0; v < G.num_vertices(); ++v) {
        ...
        for (int w = 0; w < G.num_vertices(); ++w) {
            if (spt.has_path(v, w)) { Std_out::printf("%6.2f ", spt.distance(v, w)); }
