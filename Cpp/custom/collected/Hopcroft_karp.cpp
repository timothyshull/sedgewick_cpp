...
bool Hopcroft_karp::_certify_solution(Graph& G)
{
    ...
    // check that size() is consistent with _mate()
    ...
    // check that _mate() uses each vertex at most once
    std::deque<bool>
    ...
    // check that _mate() uses only edges that appear in the graph
    for (int v = 0; v < _num_vertices; ++v) {
        if (_mate(v) == -1) { continue; }
        bool isEdge = false;
        for (int w : G.adj(v)) {
    ...
    // check that in_min_vertex_cover() is a vertex cover
    for (int v = 0; v < _num_vertices; ++v) {
        for (int w : G.adj(v)) {
